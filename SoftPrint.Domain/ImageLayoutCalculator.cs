...
{
    public static LayoutRect ComputeDestination(
        ...
        int scalePercent)
    {
        ...
        return fit switch
        {
            ImageFitMode.Stretch => new LayoutRect(pageX, pageY, pageW, pageH),
    ...
    private static LayoutRect Contain(float px, float py, float pw, float ph, float iw, float ih, float scale)
    {
        var ratio = Math.Min(pw / iw, ph / ih) * scale;
        var w = iw * ratio;
        var h = ih * ratio;
        return new LayoutRect(px + (pw - w) / 2f, py + (ph - h) / 2f, w, h);
    }

    private static LayoutRect Cover(float px, float py, float pw, float ph, float iw, float ih, float scale)
    {
    ...
    private static LayoutRect Center(float px, float py, float pw, float ph, float w, float h) =>
