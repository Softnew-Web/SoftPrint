    ...
    ILogger<WebhookNotifier> logger) : IWebhookNotifier
{
        ...
        string url, JobFinishedEventLog payload, CancellationToken cancellationToken)
    {
        try
        {
            using var client = httpClientFactory.CreateClient("softprintthook");
...
public sealed class WebhookRetryQueue(
    IAppPaths paths,
    IOptions<SoftPrintFeatureOptions> options) : IWebhookRetryQueue
{
    private readonly object _gate = new();
    private readonly List<WebhookRetryItem> _items = [];
    private readonly string _path = Path.Combine(paths.DataRoot, "webhook-retry.json");
