param([string]$Executable = "$PSScriptRoot\dist\windows-modern-x64\SoftPrint.exe")
...
$testExe = Join-Path $testFolder 'SoftPrint.exe'
$previousDataRoot = $env:SOFTPRINT_DATA_ROOT
$env:SOFTPRINT_DATA_ROOT = Join-Path $testFolder 'data'
...
function Start-TestApp {
    ...
    for ($attempt = 0; $attempt -lt 60; $attempt++) {
        ...
        try {
            ...
            $script:headers = @{ 'X-SoftPrint-Key' = $key }
...
try {
    ...
    if (-not $status.capabilities) { throw 'Capacidades da plataforma não foram retornadas.' }
    $diagnose = Invoke-RestMethod "$testUrl/api/diagnose" -Headers $headers
    if (-not $diagnose.os) { throw 'Diagnóstico não retornou o sistema operacional.' }
    $legacyHeaders = @{ 'X-AutoPrint-Key' = $headers['X-SoftPrint-Key'] }
    $null = Invoke-RestMethod "$testUrl/api/status" -Headers $legacyHeaders
    ...
    $invalidBody = @{ printerName = 'SOFTPRINT-PRINTER-DOES-NOT-EXIST'; simulation = $false; paused = $true; expectedRevision = $updated.revision } | ConvertTo-Json
    ...
    $env:SOFTPRINT_DATA_ROOT = $previousDataRoot
}
