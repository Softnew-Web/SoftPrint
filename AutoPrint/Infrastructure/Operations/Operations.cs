...
namespace AutoPrint.Infrastructure.Operations;
...
public sealed class FileLogProvider : ILoggerProvider
{
    ...
    public FileLogProvider(IHostEnvironment environment, IOptions<AutoPrintFeatureOptions> options)
    {
        _enabled = options.Value.LogToFile;
        _directory = Path.Combine(environment.ContentRootPath, "data", "logs");
    ...
    private sealed class FileLogger(string category, string directory, bool enabled, object gate) : ILogger
    {
            ...
            Func<TState, Exception?, string> formatter)
        {
            ...
            var path = Path.Combine(directory, $"autoprin-{DateTime.Now:yyyyMMdd}.log");
            lock (gate)
            {
        ...
        {
            foreach (var file in Directory.EnumerateFiles(directory, "autoprin-*.log")
    ...
    ILogger<DataMaintenanceWorker> logger) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                Backup(environment.ContentRootPath);
            }
            catch (Exception ex)
            {
    ...
    private static void Backup(string contentRoot)
    {
        var data = Path.Combine(contentRoot, "data");
